...
class websocket_connection:
    ...
    def get_qr(self, quiet_zone=1, qr_color=16):
        ...
        self.websocket.send(int(time.time()).__str__()+f',["admin", "init", [0,4,20281], ["{self.user_agent} at {datetime.datetime.now().isoformat()}", "{self.user_agent}"], "{self.session.user.clientId}", true]')
        while True:
